...
public class SmartCartServiceImpl implements SmartCartService {
	...
	public SmartCartServiceImpl(int block, String key, int terminalIndex) {
	...
	private void connect() throws CardException {
		...
		if (terminals.size() == 0)
			throw new CardException("No one terminal connected");

		if (terminalIndex > 0)
			terminal = terminals.get(terminalIndex - 1);
		else {
			if (terminals.size() == 2)
